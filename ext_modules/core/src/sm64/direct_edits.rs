...
use super::{Variable, VariableWithoutFrame};
use crate::memory::{Memory, Value};

#[derive(Debug)]
pub struct DirectEdits<M: Memory> {
    frames: Vec<HashMap<VariableWithoutFrame, Value<M::Address>>>,
}
...
impl<M: Memory> DirectEdits<M> {
    ...
    pub fn edits(
        &self,
        frame: u32,
    ) -> impl Iterator<Item = (&VariableWithoutFrame, &Value<M::Address>)> {
        self.frames.get(frame as usize).into_iter().flatten()
    }

    fn edits_mut(&mut self, frame: u32) -> &mut HashMap<VariableWithoutFrame, Value<M::Address>> {
        let index = frame as usize;
        while self.frames.len() <= index {
            self.frames.push(HashMap::new());
        }
