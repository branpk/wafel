...
pub trait GameMemory {
    ...
    type StaticView<'a>: MemoryRead where Self: 'a;
    ...
    type SlotView<'a>: MemoryRead where Self: 'a;
    ...
    /// Static addresses are looked up in shared static memory, while non-static
    /// addresses are looked up in the slot's buffer.
    type SlotViewMut<'a>: MemoryRead + MemoryWrite where Self: 'a;
    ...
    M: GameMemory + 'static,
{
    ...
    type StaticView<'a> where Self: 'a = M::StaticView<'a>;

    type SlotView<'a> where Self: 'a = M::SlotView<'a>;

    type SlotViewMut<'a> where Self: 'a = M::SlotViewMut<'a>;
